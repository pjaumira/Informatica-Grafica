...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::translate(model2, glm::vec3(0.0f, 25.0f, -30.0f));
		model = glm::scale(model, glm::vec3(50.0f, 25.0f, 20.0f));
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	...
	if (keys[GLFW_KEY_Q])
	{
		if (height_scale <= 0.0f)height_scale += 0.001;		
	}

	else if (keys[GLFW_KEY_E])
	{
		if (height_scale >= -0.06f)height_scale -= 0.001;
	}
